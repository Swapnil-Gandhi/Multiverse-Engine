...
class Scheduler(
    ...
    def __init__(
        ...
        self.init_metrics()
        # Set zombie batch
        self.zombie_batch: Optional[ScheduleBatch] = None
    ...
    def handle_generate_request(
        ...
        if (
            ...
            req = Req(
                ...
                init_input_len=len(recv_req.input_ids),
            )
    ...
    def check_memory(self):
        ...
        # memory_leak = available_size != self.max_total_num_tokens - protected_size
        if memory_leak:
            msg = (
        ...
        if self.last_batch and self.last_batch.forward_mode.is_extend():
            ...
            if not self.last_batch.is_empty():
                if self.running_batch.is_empty():
                    ...
                    self.running_batch.merge_batch(self.last_batch, log=(self.attn_tp_rank == 0))
            else:
            ...
            if not self.running_batch.is_empty():
                ...
                if self.running_batch.is_zombie and not self.running_batch.is_merge: # Split
                    if len(self.running_batch.pending_reqs) > 0:
                        ...
                        new_batch = self.get_new_batch_prefill()
                        split_batch = new_batch if new_batch is not None else None
                ...
                if self.running_batch.forward_mode is not None and self.running_batch.forward_mode.is_extend(): # In case the merge operation encounters length limit
                    if self.zombie_batch is not None:
                        zombie_batch = self.zombie_batch
                        self.zombie_batch = None
                        zombie_batch.merge_batch(self.running_batch, log=(self.attn_tp_rank == 0))
                        zombie_batch.is_merge = False
                        zombie_batch.is_zombie = False
                        if not zombie_batch.is_empty():
                            zombie_batch.prepare_for_decode()
                            ret = zombie_batch
                            self.running_batch = zombie_batch
                        else:
                            zombie_batch.filter_batch(log=(self.attn_tp_rank == 0))
                            if zombie_batch.is_merge:
                                zombie_group = zombie_batch.zombie_group
                                zombie_batch.zombie_group = []
                                assert len(zombie_group) > 0
                                self.zombie_batch = zombie_batch
                                merge_batch = self.merge_zombie_batch_to_run(zombie_group)
                                self.running_batch = ScheduleBatch(reqs=[], batch_is_full=False)
                                ret = merge_batch
                            else:
    ...
    def split_merge_zombie_batch_to_run(self, zombie_group: List[Tuple[str, List[Req]]]):
        final_reqs = []
        ...
        for _, item in enumerate(zombie_group):
            parent_rid = item[0]
            reqs = item[1]
            prefix = reqs[0].origin_input_ids + reqs[0].output_ids[:-1]
            
            over_all_list = prefix.copy()
            over_all_indices = None
            last_node_list = []
            right_most_pos = []
            parent_start_path_idx_stack = None
            ...
            for i, req in enumerate(reqs):
                if i == 0:
                    ...
                    parent_start_path_idx_stack = req.parent_start_path_idx_stack
                else:
                    full_input_ids = req.origin_input_ids + req.output_ids[:-1]
                    last_path_idx = req.start_path_idx
            ...
            new_req = Req(
                parent_rid,
                self.tokenizer.decode(over_all_list)[0],
                over_all_list,
                reqs[0].sampling_params,
                return_logprob=reqs[0].return_logprob,
                top_logprobs_num=reqs[0].top_logprobs_num,
                token_ids_logprob=reqs[0].token_ids_logprob,
                stream=reqs[0].stream,
                lora_path=reqs[0].lora_path,
                input_embeds=reqs[0].input_embeds,
                custom_logit_processor=reqs[0].custom_logit_processor,
                return_hidden_states=reqs[0].return_hidden_states,
                eos_token_ids=reqs[0].eos_token_ids,
                bootstrap_host=reqs[0].bootstrap_host,
                bootstrap_port=reqs[0].bootstrap_port,
                bootstrap_room=reqs[0].bootstrap_room,
                right_most_pos=max_right_most_pos,
                init_input_len=reqs[0].init_input_len,
                start_path_idx=parent_start_path_idx,
                parent_start_path_idx_stack=parent_start_path_idx_stack,
            )
            new_req.tokenizer = self.tokenizer
        ...
        for i, req in enumerate(final_reqs):
            req.fill_ids = req.origin_input_ids + req.output_ids
            req.prefix_indices = over_all_indices_list[i]
            req.last_node = final_last_node_list[i]
    ...
    def merge_zombie_batch_to_run(self, zombie_group: List[Tuple[str, List[Req]]]):
        final_reqs = []
        over_all_indices_list = []
        final_last_node_list = []
        for group in zombie_group:
        ...
        for _, item in enumerate(zombie_group):
            ...
            last_node_list = []
            right_most_pos = []
            path_input_ids = self.tokenizer.encode("<Path>")[0]
            end_path_input_ids = self.tokenizer.encode("</Path>")[0]
            goal_input_ids = self.tokenizer.encode("<Goal>\n")[0]
            end_goal_input_ids = self.tokenizer.encode("</Goal>\n")[0]
            parent_start_path_idx_stack = None
            for i, req in enumerate(reqs):
                if i == 0:
                    full_input_ids = prefix
    ...
    def get_new_batch_prefill(self) -> Optional[ScheduleBatch]:
        ...
        if (
            ...
            self.running_batch.filter_batch(log=(self.attn_tp_rank == 0))
            if not self.running_batch.is_empty():
    ...
    def update_running_batch(self, batch: ScheduleBatch) -> Optional[ScheduleBatch]:
        ...
        batch.filter_batch(log=(self.attn_tp_rank == 0))
        if batch.is_zombie or batch.is_merge:
            return batch
        if batch.is_empty():
        ...
        if self.zombie_batch is not None:
            batch.merge_batch(self.zombie_batch, log=(self.attn_tp_rank == 0))
            self.zombie_batch = None
